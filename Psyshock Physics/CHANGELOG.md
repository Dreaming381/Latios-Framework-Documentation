...
-   Sphere vs triangle distance algorithm has been reworked to no longer
    normalize quaternions, more accurately report hitpoints, and evaluate all
    queries faster
...
-   Point vs capsule and sphere vs capsule distance algorithms have been
    reworked to no longer normalize quaternions, more accurately report
    hitpoints, and evaluate `WithinDistance()` faster
...
-   Added `Physics` methods `AreOverlapping()` and `WithinDistance()`, which may
    be faster than `DistanceBetween()` as Psyshock queries are overhauled

### Changed

-   Point vs sphere and sphere vs sphere distance algorithms have been reworked
    to no longer normalize quaternions, and now report hitpoints computed from
    each sphere’s own center for better precision

### Fixed

-   Fixed AABB calculation for terrain colliders, which was assuming the local
    origin was at the center of the heightmap rather than the corner
...
-   Fixed distance algorithm between triangles and capsules reporting axis hits
    when the hit is actually an endpoint
-   Fixed various terrain indexing issues
...
-   Fixed an indexing issue for FindPairs using multiple `CollisionWorld.Mask`
    instances
...
-   Fixed an issue in `DistanceBetween()` for a capsule and a box that caused it
    to fail to detect collisions in some circumstances
...
-   Fixed `PhysicsDebug.DrawCollider()` for terrain colliders with holes
-   Fixed `Physics.DistanceBetween()`, `Physics.DistanceBetweenAll()` and
    `Physics.ColliderCast()` not traversing the correct terrain triangles
-   Fixed incorrect normals and contacts for queries involving triangle
    colliders in several scenarios
-   Fixed `Physics.AabbFrom()` computing the wrong result for `TerrainCollider`
-   Fixed generation of `TerrainColliderBlob` for low terrain resolutions
...
-   Fixed internal job scheduling of very large job structures, such as those
    containing a processor that uses `ComponentBroker`
