...
`AutoDestroyExpirationJournal` is a collection component on the
`worldBlackboardEntity` which contains a `NativeStream` listing destroyed
entities, and another listing entities that were removed from
`LinkedEntityGroup`. Feel free to read this collection if you need it.
