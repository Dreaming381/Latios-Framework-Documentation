...
-   Fixed `ForceInitializeUnitializedOptimizedSkeletonsSystem` never updating
    correctly because it was querying `DependentSkinnedMesh`
...
-   Fixed exception in `OptimizedSkeletonAspect` when writing to bones and not
    all sockets in the skeleton are touched
...
-   Fixed mipmap evaluation of `WorldRenderBounds` with near-zero extents
-   Fixed optimized skeleton sockets when using QVVS Transforms
-   Fixed frustum culling exceptions caused by mismanagement of the culling rate
    group allocators when the frustum culling system executes more than twice
    within a single frame

### Improved

-   Optimized mipmap streaming evaluations jobs, including an optimization that
    leverages `PromiseAllEntitiesInChunkUseSameMaterialMeshInfoTag`
...
-   Fixed Unity Transforms compilation
-   Fixed dispatch `SuperSystem`s being injected into `SimulationSystemGroup`
    and running when using an injection workflow bootstrap
...
-   Updated AclUnity plugins for linux to increase the chances of dynamic
    linking succeeding (still flaky)
...
-   Updated AclUnity Android binaries to support 16kB page sizes
...
-   *New Feature:* Added projected decal meshes, sample shaders, and various
    support logic

### Changed

-   LOD Pack no longer has checkboxes for enabling override materials, and
    instead simply evaluates if the override material list is not null and not
    empty
-   Mesh LOD components are no longer baked for meshes that only have a single
    LOD level
...
-   Fixed instantiation of a non-prefab deformed mesh entity potentially not
    having the binding system run on it

### Changed

-   The system order has changed for some systems in an explicitly-defined
    system group, which should not break projects but may appear different in
    profiling tools

### Improved

-   Improved the job scheduling behavior and performance related to computing
    the bounds of optimized skeletons when custom graphics is installed
    (typically LifeFX)
...
-   Fixed Unity Transforms mode compilation regression
...
-   Fixed Unity Transforms regression from `LatiosEntitiesGraphicsSystem`
    refactor that resulted in a container safety error
-   Fixed shadow culling SIMD indexing issue
...
-   Fixed bone propagation memory corruption issue
-   Fixed various regressions with `UnityRig.SolveTwoBoneIK()` where rotations
    were not propagating offsets correctly
