...
-   Fixed playback of `IntantiateCommandBuffers` scheduling jobs when there are
    no commands, which could cause a job system error because `EntityManager`
    was never invoked to complete jobs
...
-   Fixed corrupted playback of `InstantiateCommandBufferCommand2` buffers
-   Fixed codegen issue for VPtr involving interfaces that have multiple
...
-   Added `math` methods `InverseTransformFast()` for both points and
    `RigidTransform`s
...
-   Fixed bootstrap templates using removed `CalligraphicsBootstrap` API
-   Fixed VPtr Burst-compatibility issue caused by usage of
    `IntPtr.Equals(object)` instead of the `==` operator
